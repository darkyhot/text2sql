...
- `/build_business_report schema.table [фокус] [--where <SQL>]` — бизнес-аналитика
  таблицы → `workspace/business_report.md` (+графики seaborn). Отвечает на «что
  интересного в данных»: ТОП-ы, динамика/рост, концентрация (Парето), что выросло/
  упало, доли флагов — бизнес-языком, без матжаргона. Один запрос к БД (загрузка),
  всё остальное — pandas. Фокус (свободный текст) идёт ПЕРЕД `--where`; `--where`
  до конца строки. Примеры:
  ```
  /build_business_report s_grnplm_..._sn_uzp.uzp_dwh_fact_outflow --where report_dt >= '2026-01-01'
  /build_business_report s_grnplm_..._sn_uzp.uzp_dwh_sale_funnel_task аналитика по закрытию задач --where report_dt >= '2026-01-01'
  ```

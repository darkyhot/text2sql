...
Подключено (§8.1): маршрутизация внутри `/investigate` (`investigate._route_playbook` →
`select_playbook`), рендер каждого шага в ECharts-виджет (`surface._step_widget`), вердикт
(`impact_verdict`). Рецепты на полке: `impact` (§8.3) и `loss_attribution`.
Остаётся расширением: ad-hoc путь (LLM собирает план из примитивов, когда рецепт не подошёл) и
остальные рецепты полки (benchmark / funnel_drop / churn_risk) — добавляются JSON-ом без кода.
"""

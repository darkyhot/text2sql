...
# id-подписи не показываем как «ИНН/inn» — по требованию заменяем на «ID клиента»
_ID_LABEL_RE = re.compile(r"\bинн\b|\binn\b|\bогрн\b|\bogrn\b|\bкпп\b|\bkpp\b", re.I)
_ID_WORD_RE = re.compile(r"\bИНН\b|\bинн\b|\binn\b", re.I)


def clean_id_text(s: str) -> str:
    """Замена слова «ИНН»/«inn» на «ID клиента» в свободном тексте (эхо вопроса, ответ/цепочка LLM).
    Границы слова \\b не задевают «inner/beginning» и русские слова — HTML/JS трогать не нужно."""
    return _ID_WORD_RE.sub("ID клиента", s) if s else s
...
class Labels:
    ...
    def of(self, col: str) -> str:
        lab = self._m.get(col, col)
        return "ID клиента" if _ID_LABEL_RE.search(lab) or _ID_LABEL_RE.search(col) else lab

    def col_title(self, col: str) -> str:
        lab = self._m.get(col)
        if _ID_LABEL_RE.search((lab or "") + " " + col):     # id-колонка (инн/огрн/кпп) → без тех.имени
            return "ID клиента"
